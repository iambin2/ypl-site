...
- **Brand mark**: an app-icon tile: Braviary's (워글) head traced from its official artwork, facing left, on a `#1D1D1F` tile rounded by `clip-path: inset(0 round 23%)`. The recognition cues are kept: the upswept white crest (a fluffy back mass plus long front feathers), the forehead V (the artwork's red, `#8E8E93`) with the nose triangle above the beak (the artwork's blue, `#3A3A3C`), the open hooked beak (the artwork's yellow, `#C7C7CC`) and a round eye in a dark face. Colours are mapped to greys by brightness so the site stays neutral. The tile is identical in both themes (a `line-2` hairline marks its edge in dark mode); the favicon is the same tile.
