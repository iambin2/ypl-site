...
/* 리본은 색이 아니라 칩 체계로 구분합니다. 금색은 최고 권위 대회 하나에만.
   (이전에는 팔레트에 없는 시안·민트 rgba 를 직접 써서 두 테마 모두에서 떠 보였습니다.) */
const TIMELINE = [
...
const COMPS = [
  { tag:"매월 정규 대회", name:"파이컵", en:"", chip:"y-chip", desc:"매월 펼쳐지는 정규 대회입니다. 이곳에서 획득한 포인트로 챔피언스 시리즈 출전권을 노릴 수 있습니다." },
  { tag:"번개 이벤트", name:"파이컵 라이트", en:"", chip:"y-chip", desc:"포인트 없이 자유롭게 즐기는 번개 형식의 대회입니다. 자체적인 상품을 걸고 가볍게 진행됩니다." },
  { tag:"최고 권위", name:"챔피언스 시리즈", en:"", chip:"y-chip y-chip-laurel", desc:"한 학기 동아리의 챔피언을 결정짓는 최고 권위의 대회입니다." },
];
const LEAGUES2 = [
...
export default function AboutPage() {
  return (<section className="sec">
    ...
    <Reveal tag="h3" className="about-h"><span className="kick-line">시즌은 이렇게 흘러갑니다</span></Reveal>
    <div className="flow">
    ...
    <Reveal tag="h3" className="about-h"><span className="kick-line">YPL이 기록하는 것</span></Reveal>
    ...
    <Reveal tag="h3" className="about-h"><span className="kick-line">연혁</span></Reveal>
    ...
    <Reveal tag="h3" className="about-h"><span className="kick-line">대회</span></Reveal>
    <div className="grid g3">{COMPS.map((c,i)=>(
      <Reveal key={i} className="card hover comp" delay={i*70}>
        <div className="comp-top"><span className={c.chip}>{c.tag}</span></div>
        <h3>{c.name}</h3>{c.en&&<div className="en">{c.en}</div>}<p>{c.desc}</p>
      </Reveal>))}</div>

    <Reveal tag="h3" className="about-h"><span className="kick-line">정규 리그</span></Reveal>
    <Reveal tag="p" className="sub about-lead">2025년 6월 양분화 이후, YPL은 두 개의 정규 리그 체제로 운영됩니다.</Reveal>
