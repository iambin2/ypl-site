  ...
  return (
    <nav className={"nav"+(scrolled?" scrolled":"")}><div className="nav-in">
      ...
      <div className="nav-right">
        <button className="nav-theme" onClick={onToggleTheme} aria-label={dark?"밝은 모드로 전환":"어두운 모드로 전환"} title={dark?"밝은 모드":"어두운 모드"}><Icon n={dark?"moon":"sun"} size={18}/></button><button className="nlink admin" onClick={onAdminClick}>{admin?"로그아웃":"관리자"}</button>
