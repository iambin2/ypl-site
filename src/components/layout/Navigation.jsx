...
export const TOOL_ITEMS = [["builder","팀 빌더", true]];
      ...
      <div className={"nav-tools" + (toolsOpen ? " open" : "")} ref={toolsRef}>
        ...
        <div className="nav-tools-menu" role="menu" aria-hidden={!toolsOpen}>
          {TOOL_ITEMS.map(([key, label, beta]) => (
            <button key={key} role="menuitem" className={"nav-tools-item" + (view === key ? " on" : "")} onClick={() => navigateTool(key)}>
              <span>{label}</span>{beta && <span className="nav-tools-beta">BETA</span>}
            </button>
          ))}
        </div>
      </div>
  ...
  return (
    <div className={"nav-drawer" + (open ? " open" : "")} aria-hidden={!open}>
      ...
      <div className={"nav-tools-mobile-menu" + (toolsOpen ? " open" : "")}>
        {TOOL_ITEMS.map(([key, label, beta]) => (
          <button key={key} className={"nav-ditem nav-tools-mobile-item" + (view === key ? " on" : "")} tabIndex={open && toolsOpen ? 0 : -1} onClick={() => navigate(key)}>
            <span>{label}</span>{beta && <span className="nav-tools-beta">BETA</span>}
          </button>
        ))}
      </div>
